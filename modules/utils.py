...
def save_model(
    model, file_name="fcn_resnet50_best_model.pt", model_dir="trained_models"
):
...
def save_image(data_path, saved_path, rles, filename_and_class):
    image = cv2.imread(os.path.join(data_path, filename_and_class[0].split("_")[1]))
    preds = []
